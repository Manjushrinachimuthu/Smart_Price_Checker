...
public class CompareService {
  ...
  private static final List<String> LIVE_COMPARISON_TARGETS = List.of("Amazon", "Flipkart", "Google");
  ...
  private static final String DEFAULT_ACCEPT_HEADER = "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,*/*;q=0.8";
  private static final String DEFAULT_ACCEPT_LANGUAGE = "en-IN,en;q=0.9";
  private static final String DEFAULT_REFERER = "https://www.google.com/";
  ...
  public Map<String, Object> compare(String rawUrl) {
    ...
    out.put("liveComparison", buildLiveComparison(stores));
  ...
  private ScrapeResult scrapeProductFromUrl(String productUrl) {
    try {
      Document doc = fetchProductDocument(productUrl);
      if (doc == null) return new ScrapeResult(null, null, null, null, null, null);
  ...
  private Document fetchProductDocument(String productUrl) {
    Document doc = fetchDocumentWithJsoup(productUrl);
    return doc != null ? doc : fetchDocumentWithHttpClient(productUrl);
  }

  private Document fetchDocumentWithJsoup(String productUrl) {
    try {
      return Jsoup.connect(productUrl)
        ...
        .header("Accept-Encoding", DEFAULT_ACCEPT_ENCODING)
        .get();
    } catch (IOException e) {
      return null;
    }
  ...
  private Document fetchDocumentWithHttpClient(String productUrl) {
    try {
      HttpRequest request = HttpRequest.newBuilder(URI.create(productUrl))
      ...
      if (response.statusCode() >= 200 && response.statusCode() < 400) {
        return Jsoup.parse(response.body(), productUrl);
      }
    } catch (IOException e) {
      return null;
  ...
  private boolean isLikelyIndianStore(String storeName) {
    ...
    if (name.contains(".")) {
      List<String> allowedTlds = List.of(".in", ".co.in", ".com");
      return allowedTlds.stream().anyMatch(name::endsWith);
    }
    return true;
  }
  ...
  private String inferStoreName(Map<String, Object> item, String link) {
    ...
    private String normalizeStoreName(String name) {
      ...
      Map<String, String> aliases = Map.ofEntries(
        Map.entry("jiomart marketplace", "JioMart"),
        Map.entry("jiomart", "JioMart"),
        Map.entry("desertcart.in", "Desertcart"),
        Map.entry("desertcart", "Desertcart"),
        Map.entry("myntra.com", "Myntra"),
        Map.entry("myntra", "Myntra"),
        Map.entry("meesho.com", "Meesho"),
        Map.entry("meesho", "Meesho"),
        Map.entry("zepto.com", "Zepto"),
        Map.entry("zepto", "Zepto"),
        Map.entry("google shopping", "Google"),
  ...
  private Map<String, Object> offerToMap(Offer offer) {
    Map<String, Object> row = new LinkedHashMap<>();
    row.put("store", offer.store);
    row.put("price", offer.price);
    row.put("link", offer.link);
    row.put("title", offer.title);
    row.put("details", offer.details);
    row.put("image", offer.image);
    row.put("rating", offer.rating);
    row.put("reviewCount", offer.reviewCount);
    return row;
  }

  private List<Map<String, Object>> offersToMaps(List<Offer> offers) {
    return offers.stream().map(this::offerToMap).toList();
  }

  private List<Map<String, Object>> buildLiveComparison(List<Offer> stores) {
    List<Map<String, Object>> out = new ArrayList<>();
    if (stores == null || stores.isEmpty()) return out;
    for (String target : LIVE_COMPARISON_TARGETS) {
      stores.stream()
        .filter(o -> {
          String normalized = normalizeStoreName(o.store);
          return normalized != null && normalized.equalsIgnoreCase(target);
        })
        .findFirst()
        .ifPresent(o -> out.add(offerToMap(o)));
    }
    return out;
  }
