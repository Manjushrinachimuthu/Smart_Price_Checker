...
export default function PriceCard({ item, bestPrice, logo, onTrack, isTracked }) {
  ...
  return (
    <motion.article
      whileHover={{ y: -6 }}
      animate={isBest ? { y: [0, -5, 0], scale: [1, 1.015, 1] } : undefined}
      transition={isBest ? { duration: 2, repeat: Infinity, ease: "easeInOut" } : undefined}
      className={`card ${isBest ? "best best-animated" : ""}`}
    >
      {isBest && (
