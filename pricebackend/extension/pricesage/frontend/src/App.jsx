...
export default function App() {
  ...
  const loadAlerts = async ({ replace = false, since = null, notify = false } = {}) => {
    try {
      ...
      const res = await fetch(`http://localhost:5000/alerts?${params.toString()}`);
  ...
  useEffect(() => {
    ...
    const loadSettings = async () => {
      try {
        const res = await fetch("http://localhost:5000/notification-settings");
        const payload = await res.json();
  ...
  const saveTrackedStore = async (storeItem) => {
    ...
    const res = await fetch("http://localhost:5000/track-store", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
  ...
  const toggleTracking = async () => {
    ...
    try {
      const res = await fetch("http://localhost:5000/track-store/toggle", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  useEffect(() => {
    ...
    const syncTracked = async () => {
      try {
        ...
        const res = await fetch(`http://localhost:5000/track-store-history?${params.toString()}`);
        const payload = await res.json();
  ...
  const markAlertRead = async (id = null) => {
    await fetch("http://localhost:5000/alerts/mark-read", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
